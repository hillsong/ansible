...
short_description: Manage Extreme BOSS configuration sections
description:
  - Extreme BOSS configurations use a simple flat text file syntax.
    This module provides an implementation for working with EXOS
    configuration lines in a deterministic way.
notes:
  - Tested against BOSS 7.0.0
  ...
  parents:
    description:
      - The parent line that uniquely identifies the section the commands
        ...
        that BOSS configurations only support one level of nested commands.
  src:
    description:
...
from ansible.module_utils.network.boss.boss import run_commands, get_config
from ansible.module_utils.network.boss.boss import get_defaults_flag, get_connection
from ansible.module_utils.network.boss.boss import get_sublevel_config, BossNetworkConfig
from ansible.module_utils.basic import AnsibleModule
from ansible.module_utils.network.common.config import dumps


def get_candidate_config(module):
    candidate = BossNetworkConfig(indent=0)
...
def main():
    ...
    if module.params['backup'] or (module._diff and module.params['diff_against'] == 'running'):
        ...
        config = BossNetworkConfig(indent=0, contents=contents)
    ...
    if any((module.params['lines'], module.params['src'])):
        ...
        if match != 'none':
            ...
            config = BossNetworkConfig(contents=config, indent=0)
    ...
    if module._diff:
        ...
        running_config = BossNetworkConfig(indent=0, contents=contents,
        ...
        if contents is not None:
            base_config = BossNetworkConfig(indent=0, contents=contents,
                                            ignore_lines=diff_ignore_lines)
